...
<body>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="basic-form">
                                        <form action="/complaint/resolveHandler" method="post">
                                        	<c:if test="${complaint != null}">
                                        		<input type="hidden" name="id" value="${complaint.id}" />
                                        	</c:if>
                                            <div class="form-group">
                                                <label class="col-sm-4 control-label" for="reply">Reply</label>
                                                <textarea class="form-control" rows="6" id="reply" name="reply" required></textarea>
                                            </div>
                                            <div class="form-group text-end">
                                            	<button type="submit" onclick="return checkValidation()" class="btn btn-primary m-b-10 m-l-5">Send Reply</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
    ...
    <script>
    	function checkValidation() {
			...
			if(reply == '') {
				toastr.error('Please Enter Valid Reply !!','Invalid Inputs',{
