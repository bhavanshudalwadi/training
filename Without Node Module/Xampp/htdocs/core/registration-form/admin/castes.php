<?php
    ...
    $result = $caste->get_by_page($page, $records_per_page);
    $count = $caste->get_count();
...
<body>
                    ...
                    <div class="card-body">
                        <div class="row mt-2 mb-3">
                            ...
                            <div class="col-md-6 d-flex justify-content-end">
                                <a href="<?php echo app_url; ?>/edit-caste.php" class="btn btn-primary">Add Caste</a>
                            </div>
                        </div>
                            ...
                            <tbody>
                                ...
                                <?php while($row = mysqli_fetch_assoc($result)): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="<?php echo app_url; ?>/edit-caste.php?id=<?php echo $row['id']; ?>" class="btn btn-outline-success">Edit</a>
                        ...
                        <nav aria-label="Page navigation" class="d-flex justify-content-between">
                            ...
                            <ul class="pagination justify-content-end m-0">
                                <li class="page-item"><a class="page-link <?php echo ($page <= 1)?"disabled":""; ?>" href="<?php echo ($page > 1)?app_url."/castes.php?page=".($page-1)."&per_page=".$records_per_page:"#"; ?>">Previous</a></li>
                                <?php foreach(range(1, $pages) as $p): ?>
                                    <li class="page-item"><a class="page-link" href="<?php echo app_url; ?>/castes.php?page=<?php echo $p; ?>&per_page=<?php echo $records_per_page; ?>"><?php echo $p; ?></a></li>
                                <?php endforeach; ?>
                                <li class="page-item"><a class="page-link <?php echo ($page >= $pages)?"disabled":""; ?>" href="<?php echo ($page < $pages)?app_url."/castes.php?page=".($page+1)."&per_page=".$records_per_page:"#"; ?>">Next</a></li>
                            </ul>
                        </nav>
                    </div>
                </div>
    ...
    <script>
        $("#per_page").on('change', function() {
            ...
            window.location.href = `<?php echo app_url; ?>/castes.php?page=1&per_page=${per_page}`;
		});

        function deleteCaste(name, id) {
            if(confirm(`Are you sure want to delete caste named \`${name}\``)) {
                window.location.href = `<?php echo app_url; ?>/actions/delete-caste.php?id=${id}`;
            }
        }
    </script>
</body>
